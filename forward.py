...
def main():
    ...
    my_own_test(model, "data/testdirs")
...
def my_own_test(model, dataDirs):
    model.eval()
    ...
    for image in os.listdir(dataDirs):
        imgPIL = Image.open(join(dataDirs, image)).convert('RGB')
        img = transform(imgPIL)
        img = img.unsqueeze(0)
        img = img.cuda()
        key_points = model(img)
        key_points = torch.sigmoid(key_points)
        binary_kmap = key_points.squeeze().cpu().detach().numpy() > CONFIGS['MODEL']['THRESHOLD']
        ...
        vis = visulize_mapping(b_points, size[::-1], join(dataDirs, image))
        cv2.imwrite(join(CONFIGS["MISC"]["TMP"], image), vis)
        np_data = np.array(b_points)
        np.save(join(CONFIGS["MISC"]["TMP"], image.split('.')[0]), np_data)
        if CONFIGS["MODEL"]["EDGE_ALIGN"]:
            for i in range(len(b_points)):
                b_points[i] = edge_align(b_points[i], image, size, division=5)
            vis = visulize_mapping(b_points, size, image)
            cv2.imwrite(join(CONFIGS["MISC"]["TMP"], image.split('.')[0] + '_align.png'), vis)
            np_data = np.array(b_points)
            np.save(join(CONFIGS["MISC"]["TMP"], image.split('.')[0] + '_align'), np_data)
        return
